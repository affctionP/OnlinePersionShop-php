...
include('admin_area/db.php');
	...
	{
		$order_total_price = $p_ip['total_price_purchase'];
	}
    ...
    $query="INSERT INTO `order` (`order_total_price`,`order_is_verified`,`order_email_customer`)
    VALUES ('$order_total_price','false','$order_email')";
    $run_order=mysqli_query($con,$query);
    //select from order all order by email of customer and the
    //last one is get and save in a session named order_id and will sent to request
